...
internal class CellImageView : AppCompatImageView {
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        ...
        try {
            a?.apply {
                isBitmap = getBoolean(R.styleable.CellImageView_btm_imageview_isBitmap, isBitmap)
                useColor = getBoolean(R.styleable.CellImageView_btm_imageview_useColor, useColor)
                resource = getResourceId(R.styleable.CellImageView_btm_imageview_resource, resource)
                color = getColor(R.styleable.CellImageView_btm_imageview_color, color)
                size = getDimensionPixelSize(R.styleable.CellImageView_btm_imageview_size, size)
                actionBackgroundAlpha = getBoolean(R.styleable.CellImageView_btm_imageview_actionBackgroundAlpha, actionBackgroundAlpha)
                changeSize = getBoolean(R.styleable.CellImageView_btm_imageview_changeSize, changeSize)
                fitImage = getBoolean(R.styleable.CellImageView_btm_imageview_fitImage, fitImage)
            }
        } finally {
            a?.recycle()
        }
